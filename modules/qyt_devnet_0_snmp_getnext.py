...
def get_ifs(ip, type, community):
    ...
    cursor = conn.cursor()
    cursor.execute("select interfaces_name_oid, interfaces_bw_oid, interfaces_in_bytes_oid, interfaces_out_bytes_oid from qytdb_devicetypesnmp where type='" + type + "'")
    result = cursor.fetchall()
    # 获取接口名字列表
    ifs_name_list_raw = snmpv2_getnext(ip, community, result[0][0], port=161)
    # 获取接口物理带宽列表
    ifs_bw_list_raw = snmpv2_getnext(ip, community, result[0][1], port=161)
    # 获取入向字节数列表
    ifs_in_list_raw = snmpv2_getnext(ip, community, result[0][2], port=161)
    # 获取出向字节数列表
    ifs_out_list_raw = snmpv2_getnext(ip, community, result[0][3], port=161)
    # 产生接口名称的列表
    if type == "ASA Firewall":
...
if __name__ == "__main__":
    ...
    print(get_ifs(device_ip, 'IOS Router', community))
    print(get_ifs(nexus_ip, 'Nexus Switch', community))
    print(get_ifs(asa_ip, 'ASA Firewall', community))
